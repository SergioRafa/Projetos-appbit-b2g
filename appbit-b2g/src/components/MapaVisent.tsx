...
// Dados simulados do MVP alinhados com os contratos da API do Pedro
const dadosRegioes = [
  {
    id: 1,
...
export default function MapaVisent() {
  // Centro estratégico no Vale do Paraíba para mostrar SP e RJ na mesma tela inicial
  const posicaoCentro: [number, number] = [-23.30, -45.30]

  return (
    <MapContainer 
      center={posicaoCentro} 
      zoom={7} // Zoom 7 é perfeito para enxergar o eixo SP-RJ de uma vez só
    ...
    >
      ...
      {/* Mapeando os dados para gerar os indicadores visuais e marcadores */}
      {dadosRegioes.map((regiao) => {
        // Define a cor do indicador visual: Vermelho para crítico, Verde para estável
        const corIndicador = regiao.status === 'critico' ? '#ef4444' : '#22c55e';

        return (
          <Fragment key={regiao.id}>
            {/* 1. O Indicador Visualizado (Exigência do MVP) */}
            {/* Criamos uma mancha translúcida ao redor da região para destacar a situação */}
            <Circle
              center={regiao.posicao}
              radius={25000} // Raio de 25km para dar destaque no mapa
              pathOptions={{
                color: corIndicador,
            ...
            {/* 2. O Marcador Interativo com os dados detalhados */}
            <Marker position={regiao.posicao} icon={IconeCustomizado}>
              <Popup>
                <div style={{ color: '#000', fontFamily: 'sans-serif', minWidth: '180px' }}>
                  ...
                  {/* Tag customizada com a cor do status do indicador */}
